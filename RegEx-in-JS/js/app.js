...
function isValidUsername(username) {
  ...
  // /xxx/ regEx
  // [a-z] only lowercase letter one only, * or + means repeat for all
  // ^xxx$ means we only want what is inside this, beginning and end of string
}
...
function isValidPassword(password) {
  return  /[A-Z]/.test(password) &&
          /[a-z]/.test(password) &&
          /[0-9]/.test(password);
  //look ahead: /^(?=.*\d)(?=.*[a-z])(?=.[A-Z]).*$/.test(password)
}
